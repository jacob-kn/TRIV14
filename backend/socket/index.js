...
const setupWebSocket = (io) => {
  io.on("connection", (socket) => {
    ...
    socket.on("joinRoom", (roomCode, username) => {
      if (io.sockets.adapter.rooms.has(roomCode)) {
        // making sure the room exists before joining
        ...
        io.to(roomCode).emit(
          "updatedUserList",
          Array.from(usersInRoom.values())
        ); // sending client a list of all users in room
    ...
    socket.on("startQuiz", (quiz, roomCode, duration) => {
      // quiz is literally the json object
    ...
    socket.on("socketToUser", (scoreList, roomCode) => {
      const userInRoom = roomData.get(roomCode);
      const userScores = {};
      for (const [socketId, score] of Object.entries(scoreList)) {
        const username = usersInRoom.get(socketId);
        if (username) {
          userScores[username] = score;
        }
      }
      const mapSort = new Map([...userScores.entries()].sort((a, b) => b[1] - a[1]));

      // Now userScores contains the mapping of usernames to scores
      // You can emit this to the room or handle it as needed
      io.to(roomCode).emit("finalScores", mapSort);
    });
  });
};
